...
divider_set = create_dataset_for_autoencoder.create_dataset_divider(args, index_shuffle, ratio_train = [0.01, 0.99, 1])
...
if if_generate_output:
    ...
    for n in tqdm(range(len(list_pickle_file))):
        ...
        if args.model == "autoencoder":
            process_data.generate_output(args, epoch, model, criterion, device, gen_dataset, norm_scale)        
        else:                
            process_data.generate_output_DNN(args, epoch, model, criterion, device, gen_dataset, norm_scale)
